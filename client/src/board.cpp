...
Board::Board(sf::Vector2i window_size, int menu_height) {
    m_cell_amount = 10;
    ...
    m_cell_size.y = (m_window_size.y - menu_height - m_boarder_size.y) / m_cell_amount;
    ...
    for (int row = 0; row < m_cell_amount; row++) {
        for (int column = 0; column < m_cell_amount; column++) {
            m_board[row][column].set_type(CellTextures::Default,
            ...
            m_board[row][column].get_label()->setPosition(
                m_board[row][column].get_cell()->getPosition()
            );
