...
Unit::Unit(UnitType unit_type, sf::Vector2f position, sf::Vector2f size) {
    ...
    m_table.setPosition(sf::Vector2f(position.x + 3 * size.x / 4, position.y + 3 * size.y / 4));
    ...
    m_label.setPosition(sf::Vector2f(position.x + 13 * size.x / 16, position.y + 3 * size.y / 4));
}
